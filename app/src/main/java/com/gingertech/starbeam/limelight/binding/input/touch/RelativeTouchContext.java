package com.gingertech.starbeam.limelight.binding.input.touch;
...
public class RelativeTouchContext implements TouchContext {
    ...
    private final Runnable dragTimerRunnable = new Runnable() {
        ...
        public void run() {
            ...
            if(UserData.currentLayout.isMouseClickEnabled) {
    ...
    private void checkForConfirmedMove(int eventX, int eventY) {
        ...
        distanceMoved += Math.sqrt(pow(eventX - lastTouchX, 2) + pow(eventY - lastTouchY, 2));
            ...
            if (actionIndex == 0) {
                ...
                if (pointerCount == 2) {
                    ...
                    if (!UserData.currentLayout.isTouchTrackpad) {
                        ...
                        double multiplier = pow(2 * UserData.currentLayout.touchSensitivity / 0.5, 2);

                        conn.sendMouseMove(
                                (short) (deltaX * multiplier),
                                (short) (deltaY * multiplier));
                    }
                }
